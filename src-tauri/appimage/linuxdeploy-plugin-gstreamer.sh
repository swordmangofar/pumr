...
# pumr copy of tauri-apps/linuxdeploy-plugin-gstreamer (master 2a2e6749), the
# plugin the Tauri bundler downloads into its tools cache for
# bundleMediaFramework unless a copy is already there. CI installs this file
# first, like linuxdeploy-plugin-gtk.sh:
#
#   install -D -m 0755 src-tauri/appimage/linuxdeploy-plugin-gstreamer.sh \
#     "${XDG_CACHE_HOME:-$HOME/.cache}/tauri/linuxdeploy-plugin-gstreamer.sh"
#
# Local change, marked "PUMR": the linuxdeploy run that deploys the plugins'
# libraries excludes libwayland-*. This plugin runs after the GTK plugin, which
# removed the bundled libwayland; that run would otherwise copy it back in for
# GTK and WebKit.
...
# PUMR: keep libwayland out, see linuxdeploy-plugin-gtk.sh.
"$LINUXDEPLOY" --appdir "$APPDIR" --exclude-library 'libwayland-*'
