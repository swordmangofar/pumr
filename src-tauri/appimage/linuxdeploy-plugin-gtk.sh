...
# pumr copy of tauri-apps/linuxdeploy-plugin-gtk (master dda522bc), the plugin
# the Tauri bundler downloads into its tools cache unless a copy is already
# there. CI installs this file first so AppImages are built with it:
#
#   install -D -m 0755 src-tauri/appimage/linuxdeploy-plugin-gtk.sh \
#     "${XDG_CACHE_HOME:-$HOME/.cache}/tauri/linuxdeploy-plugin-gtk.sh"
#
# Local changes, marked "PUMR":
#   * GDK_BACKEND is no longer forced to x11, so the AppImage runs natively on
#     Wayland like the .deb/.rpm. XWayland windows are bitmap-scaled by the
#     compositor on scaled displays, which makes all text blurry.
#   * the bundled libwayland-* are removed, see the end of this file.
...
# PUMR: GDK_BACKEND is not forced to x11 (upstream does because of
# https://github.com/tauri-apps/tauri/issues/8541, an AppImage built on
# Ubuntu 20.04), so GTK picks native Wayland when available and X11 otherwise.
# The NVIDIA driver keeps x11 unless GDK_BACKEND is set: WebKitGTK on
# NVIDIA + Wayland can leave the window blank
# (https://v2.tauri.app/develop/debug/linux-graphics/).
if [ -z "${GDK_BACKEND:-}" ] && [ -e /proc/driver/nvidia/version ]; then
...
# PUMR: never ship libwayland. The build host's copy shadows the system one,
# and a newer Mesa then cannot create an EGL display (missing wl_* symbols):
# WebKitWebProcess aborts with EGL_BAD_PARAMETER and the window stays blank
# (https://github.com/tauri-apps/tauri/issues/15665). Mesa and GTK make every
# desktop provide libwayland.
